...
#Sends request to OpenAI to generate four links that contains AI images generated by DALL-E      
def image_generate(x):
    #WARNING: Newly generated API keys were not accepted by DALL-E. We hope that this key still has some free charges on it.
    openai.api_key = "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"     
...
#Sends request to AZLyrics API to obtain the lyrics based on the sent song name. For accuracy, the duckduckgo engine is used, in case the name is not written perfectly.
...
#To be added later for a different selection of keywords.
...
#Processes the input text and returns the top phrase or keywords with the highest significance
